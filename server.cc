            ...
            if (FD_ISSET(fd, &read_fds)) {
                if (fd == tcp_fd) {
                    ...
                    if (size < 0)
                        ...
                        if (msg == ":quit") {
                            std::string dc_msg(userbyfd.at(fd) + " left");
                            close(fd);
                            FD_CLR(fd, &active_fds);
                            userbyfd.erase(fd);
                            broadcast(active_fds, dc_msg);
                        } else if (msg == ":who") {
                            std::string list_msg("Participants are ");
                            for (auto& u : userbyfd)
                                list_msg += u.second + " ";
                            broadcast(active_fds, list_msg, false);
                        } else {
                            std::string titled_msg(userbyfd.at(fd) +":"+ msg);
                            broadcast(active_fds, titled_msg);
                        }
                    }
                }
            }
...
void Server::broadcast(const fd_set& fds, const std::string& msg, bool log)
{
    ...
    for (int other_fd = 0; other_fd < FD_SETSIZE; ++other_fd)
        if (FD_ISSET(other_fd, &fds) && other_fd != tcp_fd)
